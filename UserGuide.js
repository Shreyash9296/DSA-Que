...
function UserGuide() {
  ...
  const processHtmlContent = (html) => {
    ...
    for (let i = 0; i < allParagraphs.length; i++) {
      ...
      if (inContentsSection) {
        ...
        const hasOnlyLink =
          links.length > 0 &&
          element.textContent.trim().match(/^\d+(\.\d+)*\./);

        // Check if this is actual content (has Toc ID directly on it, not in a link)
        const isActualContent =
          (element.id && element.id.startsWith('Toc')) ||
    ...
    elementsWithIds.forEach((el, index) => {
      ...
      if (index < 10) {
        console.log(
          `Element ${index}: id="${el.id}", tag="${el.tagName}", text="${el.textContent.substring(
            0,
            50
          )}..."`
        );
  ...
  const loadDocumentContent = async () => {
    try {
      ...
      setHtmlContent(
        '<div class="p-8 text-red-600">Error loading document. Please refresh the page.</div>'
      );
      setLoading(false);
    }
  };
  ...
  const handleSectionClick = (sectionId) => {
    ...
    setTimeout(() => {
      ...
      console.log(
        `Found ${allElementsWithId.length} elements with id="${sectionId}"`
      );
      ...
      allElementsWithId.forEach((el, index) => {
        console.log(
          `  Element ${index}: ${el.tagName}, parent: ${el.parentElement?.tagName}, display: ${window.getComputedStyle(
            el
          ).display}`
        );
      ...
      if (targetElement) {
        console.log(
          `Scrolling to: ${targetElement.tagName} - "${targetElement.textContent.substring(
            0,
            50
          )}..."`
        );
        ...
        const scrollPosition =
          currentScroll + (elementRect.top - containerRect.top) - offset;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      <header
        className="sticky top-0 z-50 h-14 flex items-center px-6 shadow-sm"
        style={{ backgroundColor: 'rgb(29, 38, 44)' }}
      >
              ...
              filteredNav.map(section => (
                <div key={section.id} className="mb-1">
                  <button
                    ...
                    style={{
                      backgroundColor:
                        activeSection === section.id ? '#ffebee' : 'transparent',
                      color:
                        activeSection === section.id ? '#DB0011' : '#374151',
                    }}
                  >
                    <span
                      className="flex-shrink-0 w-5 h-5 flex items-center justify-center text-xs transition-transform"
                      style={{
                    ...
                    <span
                      ...
                      style={{ backgroundColor: '#DB0011', color: 'white' }}
                    >
                      {section.number}
                    </span>
